...
async function up(knex) {
    return knex.schema
        ...
        .then(() => knex.schema.createTable('userProfileFavouriteGyms', function (table) {
        ...
        .then((users) => Promise.all(users.map((u) => knex('userProfiles').insert({
        name: u.name,
...
async function down(knex) {
    return knex.schema
        ...
        .then((userProfiles) => Promise.all(userProfiles.map((u) => knex
        .update({
        name: u.name,
...
exports.down = down;
//# sourceMappingURL=20221014035414_update-user-tables.js.map